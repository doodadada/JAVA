...
public class Application {
    public static void main(String[] args) {
        ...
        * */
        ...
        /*
        *  이러한 상속이라는 기술을 사용하게 되면 얻게 되는 이점은 두 가지가 잇다.
        *  1. 새로운 클래스를 작성시 기존에 작성한 클래스를 재사용할 수 있다.
        *   1-1. 재사용 시 생산성을 크게 향상시킬 수 있다. (새롭게 작성하는 것보다 빠르다)
        *   1-2. 공통적으로 사용하는 코드가 부모클래스에 존재하면 수정사항이 생길 시 부모 클래스만 수정해도 전제적으로
        *        적용된다(유지 보수성 증가)
        ...
        *     상속 구조가 복잡해질수록 그 영향에 대한 예측이 힘들어 이런 단점이 유지보수성이 증가한다는 장점과는 반대ㅗㄹ
        *     유지보수에 악영향을 미친다.
        *  2. 또한 부모클래스 변경 또한 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능인 경우 부모클래스 변경시
        *     자식 클래스에 모두 영향을 줄 수 잇다. 역시 유지보수에 악영향을 미친다.
        ...
        * */
        ...
        /*
        *  객체지향 설계 관점에서 비례되는 상속
        *  모든 객체는 자신이 수신한 메세지에 대한 응받을 해야하는 책임을 가지고, 그 책임의 규모는 적절해야한다.
        *  적절한 책임을 가진 객체들이 서로 협력(메세지 수진과 응답)을 통해 프로그램이 동작하는 것을
        *  객체지향 프로그램이라고 한다.
        * */
        ...
        *  부모 클래스를 추상화 하는 경우 역할의 관점으로 바라봐야 한다.
        *  그래야 자식클래스로 생성한 객체들이 서로 역할을 수행해가며 유연한 코드를 적성할 수 ㅇ;다.
        *  동일한 역할을 가지는 모든 객체는 동일한 메세지를 수신하지는 않지만 객체별로 그 메시지에 응답하는 방식이
        *  서로 다를 수 있다.
        * */
        Car car = new Car();
        car.soundHorn();
