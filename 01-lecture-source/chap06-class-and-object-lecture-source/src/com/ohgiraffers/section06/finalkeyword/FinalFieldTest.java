...
public class FinalFieldTest {
    ...
    *  따라서 초기 인스턴스가 생성 되고 나면 기본값 0이 필드에 들어가게 되는데
    *  그 초기화 이후 값을 변경할 수 없기 때문에 선언을 하면서 바로 초기화를 해주어야 한다.
    *  그렇지 않으면 compile error가 발생한다.
    * */

    /* 선언과 동시에 초기화를 해주어야 한다.*/
    // private final int NON_STATIC_NUM; //final 키워드를 사용하였는데 초기화를 바로 하지 않아서 에러발생
    private final int NON_STATIC_NUM = 1; //final 키워드를 사용하였는데 초기화를 바로 하지 않아서 에러발생
    ...
    * */
    ...
    //private static final int STATIC_NUM; //에러발생 0으로 초기화 되기 때문에 이후 값 변경 불가
    private static final int STATIC_NUM = 1;
    private static final double STATIC_DOUBLE;
//    public FinalFieldTest(int STATIC_NUM){
//        this.NON_STATIC_NUM = STATIC_NUM;
//    }
