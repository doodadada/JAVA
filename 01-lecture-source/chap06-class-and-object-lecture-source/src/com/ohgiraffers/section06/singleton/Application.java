...
public class Application {
    public static void main(String[] args) {
        ...
        /*
        *  singleton-pattern이란
        *  애플리케이션이 시작될 때 어떤 클래스가 최초 한 번만 메모리를 결정하고 그 메모리에 인스턴스를 만들어서
        *  하나의 인스턴스를 공유해서 사용하며 메모리 낭비를 방치할 수 있게 함(매번 인스턴스 생성 하지 않음)
        *  장점
        *  1. 첫 번째 이용시에는 인스턴스를 생성해야하므로 속도 차이가 나지 않지만
        *     두 번째 이용시에는 인스턴스 생성 시간 없이 사용할 수 있다.
        *  2. 인스턴스가 절재덪긍로 한 개만 존재하는 것을 보증할 수 있다.
        *  단점
        ...
        *  */
        ...
        // EagerSingleton eager = new EagerSingleton();//에러발생 생성자가 private이라서 접근 불가  
