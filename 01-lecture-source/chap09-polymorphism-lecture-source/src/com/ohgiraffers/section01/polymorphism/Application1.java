...
public class Application1 {
    public static void main(String[] args) {
        ...
        /*
        *  다형성이란?
        *  하나의 인스턴스가 여러가지 타입을 가질 수 있는 겻을 의미한다.
        *  그렇게 때문에 하나의 타입으로 여러타입의 인스턴스를 처리할 수 있기도 하고,
        ...
        *     이런 동일한 매세지를 수신받아 처리하는 내용을 객체별로 다르게 할 수 있다는 장점을 가지고 있다.
        *     (다양한 기능을 사용하는데 있어서 관리해야할 메세지 종류가 줄어들게 된다)
        *     하나의 호출로 여러가지 동작을 수행할 수 있따는 측면에서 오버라이딩을 다형성이라고 보기도 한다.
        *     다형성을 이해하기 쉬운 가장 좋은 예이기도 하다.
        *     하지만 이 부분은 이견이 많아 존재하기 때문에 다형성을 이해하는데 참고로만 사용하자.
        ...
        * */
        ...
        //a1.jump(); //에러발생, 레퍼런스 타입이 Animal이라서 rabbit 고유의 메소드는 호출불가
        //a2.bite(); //에러발생
        ...
        ((Tiger)a2).bite();
        // ((Tiger)a1).jump(); //에러 발생
        // ((Tiger)a1).bite(); //컴파일때는 괜찮은데 실행하면 에러발생, 토끼는 호랑이가 될 수 없다.
        ...
        *  또한 작성 여부따라 명시적과 묵시적 두 가지로 구분된다.
        * */

        Animal animal1 = new Rabbit(); //up-casting 묵시적 형 변환
        Animal animal2 = (Animal) new Rabbit(); //up-casting 명시적 형 변환

        Rabbit rabbit1 = (Rabbit) animal1; //down-casting 명시적 형변환
        // Rabbit rabbit2 = animal2; //에러발생, down-casting은 묵시적 형변환이 안됨
    }
}
